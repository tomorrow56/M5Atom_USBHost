...
//                USB_SPI.begin(); // The SPI library with transaction will take care of setting up the pins - settings is set in beginTransaction()
                // SPI.begin(SCK, MISO, MOSI, SS) for M5Atom
                USB_SPI.begin(33, 19, 23, 32); // The SPI library with transaction will take care of setting up the pins - settings is set in beginTransaction()
                SPI_SS::SetDirWrite();
                SPI_SS::Set();
        }
...
//for M5Atom
typedef SPi< P33, P23, P19, P32 > spi;
#else
